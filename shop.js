...
document.querySelectorAll('.cart-btn:not(.acc-btn)').forEach(b=>b.addEventListener('click',openDrawer));
document.getElementById('drClose').onclick=closeDrawer;
overlay.onclick=()=>{closeDrawer();closeCheckout();closeAuth()};
function renderCart(){
...
document.getElementById('toCheckout').onclick=openCheckout;
...
const langOpts=[...document.querySelectorAll('.lang-row button[data-lang]')];
...
function setLang(l){
  ...
  refreshLangNodes();
  langOpts.forEach(b=>{const on=b.dataset.lang===l;if(on)b.setAttribute('aria-current','true');else b.removeAttribute('aria-current')});
  renderCart();
}
langOpts.forEach(b=>b.addEventListener('click',()=>setLang(b.dataset.lang)));
setLang(lang);
window.HT={setLang};
/* ---------- Konto (Anmelden / Registrieren) ---------- */
const authModal=document.getElementById('authModal');
function openAuth(){closeDrawer();authModal.classList.add('open');overlay.classList.add('show');document.body.style.overflow='hidden'}
function closeAuth(){if(!authModal)return;authModal.classList.remove('open');overlay.classList.remove('show');document.body.style.overflow=''}
if(authModal){
  const accBtn=document.getElementById('accBtn'),authMsg=document.getElementById('authMsg'),confirmNote=document.getElementById('authConfirm');
  const sbtBtn=document.getElementById('sbtBtn'),authView=document.getElementById('authView'),accView=document.getElementById('accView'),accDot=document.getElementById('accDot');
  const tLogin=document.getElementById('tLogin'),tReg=document.getElementById('tReg');
  let mode='login',sb=null;
  const setMode=m=>{mode=m;authModal.classList.toggle('reg',m==='reg');tLogin.classList.toggle('on',m==='login');tReg.classList.toggle('on',m==='reg');
    document.getElementById('fPass').setAttribute('autocomplete',m==='reg'?'new-password':'current-password');
    authMsg.textContent='';authMsg.className='auth-msg';confirmNote.hidden=true};
  tLogin.onclick=()=>setMode('login');tReg.onclick=()=>setMode('reg');
  document.querySelectorAll('.authSwitch').forEach(b=>b.onclick=()=>setMode(mode==='reg'?'login':'reg'));
  accBtn.onclick=openAuth;
  document.getElementById('authClose').onclick=closeAuth;
  document.addEventListener('keydown',e=>{if(e.key==='Escape')closeAuth()});
  if(window.supabase){
    sb=window.supabase.createClient('https://bldsaomuxhrujtxxpvha.supabase.co','sb_publishable_0kyqFlGKvps5jIZ6Nj8cLQ_TWGerf7k',{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,storageKey:'ht-auth'}});
  }
  const fail=t=>{authMsg.className='auth-msg err';authMsg.textContent=t};
  async function showAccount(){
    if(!sb)return false;
    const ses=(await sb.auth.getSession()).data.session;
    if(!ses)return false;
    const u=ses.user,nm=(u.user_metadata&&(u.user_metadata.name||u.user_metadata.full_name))||u.email.split('@')[0];
    const via=(u.app_metadata&&u.app_metadata.provider)||'email';
    authView.style.display='none';accView.style.display='block';
    document.getElementById('accAvatar').textContent=(nm||'?').trim().charAt(0).toUpperCase()||'?';
    document.getElementById('accName').textContent=nm;
    document.getElementById('accEmail').textContent=u.email+(via==='google'?' · Google':'');
    accDot.hidden=false;
    return true;
  }
  function showAuthForm(){authView.style.display='';accView.style.display='none';accDot.hidden=true;setMode('login')}
  document.getElementById('logoutBtn').onclick=async()=>{if(sb)await sb.auth.signOut();showAuthForm()};
  document.getElementById('googBtn').onclick=async()=>{
    if(!sb)return fail('Auth service unavailable.');
  ...
  document.getElementById('authForm').addEventListener('submit',async e=>{
    e.preventDefault();
    if(!sb)return fail('Auth service unavailable.');
    const email=document.getElementById('fEmail').value.trim().toLowerCase(),pass=document.getElementById('fPass').value;
    const name=document.getElementById('fName').value.trim();
    sbtBtn.disabled=true;authMsg.textContent='';authMsg.className='auth-msg';confirmNote.hidden=true;
    try{
      if(mode==='reg'){
        const r=await sb.auth.signUp({email,password:pass,options:{data:{name:name||email.split('@')[0]}}});
        if(r.error)throw r.error;
        if(!r.data.session){confirmNote.hidden=false;sbtBtn.disabled=false;return}
      }else{
        const r=await sb.auth.signInWithPassword({email,password:pass});
        if(r.error)throw r.error;
      }
      await showAccount();setTimeout(closeAuth,900);
    }catch(err){fail((err&&err.message)||'Error');}
    sbtBtn.disabled=false;
  });
  if(sb){sb.auth.onAuthStateChange((_e,ses)=>{ses?showAccount():showAuthForm()});showAccount();
    if(/access_token=/.test(location.hash))openAuth();}
}
})();
